...
protected:
    ...
    double tau_Ca, tau_Ca_q10, ref_temp;
...
public:
    ...
    // constructor
    compartment(double v_init, double Ca_init, double tauCa, double tauCaq10, double rtemp)
    {
        state = compstate(v_init,Ca_init);
        tau_Ca = tauCa;
        i_ext = 0.0;
...
double compartment::integrate(double dt)
{
    ...
    state.set_Ca(Ca_inf + (Ca - Ca_inf)*exp(-dt/(tau_Ca*pow(tau_Ca_q10,((ref_temp-get_T())/10.0)))));
...
// get voltage, [Ca], temp
void compartment::get_vct(double* out)
{   
    out[0] = state.get_v();
    out[1] = state.get_Ca();
    out[2] = state.get_T();
...
void compartment::set_T(double T_in)
{
    state.set_T(T_in);
}

double compartment::get_T(void)
{
    return state.get_T();
}
