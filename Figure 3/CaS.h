...
public:
    ...
    //specify both gbar and erev and temp dependencies
    CaS(double g_in, double e_in, double gq10, double actq10, double inactq10, double reftemp)//: conductance(g_in)
    {
        gbar = g_in;
        state_dim = 2;
        e_rev = e_in;
...
void CaS::integrate(double dt)
{
    ...
    h = h_inf() + (h - h_inf())*exp(-dt/(tau_h()*pow(inact_q10,((ref_temp-get_T())/10.0))));
    g = gbar*m*m*m*h*pow(g_q10,((get_T()- ref_temp)/10.0));
